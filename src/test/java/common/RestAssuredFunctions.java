...
public class RestAssuredFunctions {
    ...
    public static Response post (String url, String accessToken, Object requestBody){
        return given().contentType(ContentType.JSON).header("Authorization", "Bearer "+ accessToken).body(requestBody).post(url).then().extract().response();
    }

    public static Response put (String url, String accessToken, Object requestBody){
        return given().contentType(ContentType.JSON).header("Authorization", "Bearer "+ accessToken).body(requestBody).put(url).then().extract().response();
    }

    public static Response patch (String url, String accessToken, Object requestBody){
        return given().contentType(ContentType.JSON).header("Authorization", "Bearer "+ accessToken).body(requestBody).patch(url).then().extract().response();
    }

    public static Response delete (String url, String accessToken){
        return given().contentType(ContentType.JSON).header("Authorization", "Bearer "+ accessToken).delete(url).then().extract().response();
    }
}
